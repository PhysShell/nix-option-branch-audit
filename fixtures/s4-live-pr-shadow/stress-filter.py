#!/usr/bin/env python3
"""S4-B: stress-eligibility filter over the pool remaining after
S4-A's own frozen order is removed.

EXACT same 6 categories as S2-B/S3-B, unchanged, per the protocol's
own explicit "reused verbatim, never re-tuned" requirement.
"""
import json
import os
import re
...
remaining = json.loads(Path("/tmp/s4-draw-remaining.json").read_text())
...
def main():
    ...
    (out_dir / "s4b-stress-eligible.json").write_text(
        json.dumps([r for r, m in stress_eligible], indent=2)
    )
    (out_dir / "s4b-stress-eligible-with-cats.json").write_text(
        json.dumps(
            [{"number": r["number"], "title": r["title"], "categories": m} for r, m in stress_eligible],
            indent=2,
        )
    )
