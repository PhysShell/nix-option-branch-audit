...
with open('/tmp/s2-remaining-for-stress.json') as f:
    remaining = json.load(f)

# real, added/removed-line-only regex categories, pre-registered exactly
# as given by the user, OR-ed, no post-hoc narrowing.
categories = {
    "option_declaration": re.compile(r'\b(mkOption|mkEnableOption)\b'),
    "nixos_tests_touched": None,  # handled separately via file paths
...
for r, m in stress_eligible:
    for c in m:
        cat_counts[c] += 1
print("category hit counts (a PR can match multiple):")
for c, n in cat_counts.most_common():
    print(f"  {c}: {n}")

with open('/tmp/s2-stress-eligible.json', 'w') as f:
    json.dump([r for r, m in stress_eligible], f, indent=2)
with open('/tmp/s2-stress-eligible-with-cats.json', 'w') as f:
