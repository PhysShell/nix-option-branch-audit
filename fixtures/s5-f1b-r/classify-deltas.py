#!/usr/bin/env python3
"""S5-F1B-R: mechanical classification of every changed PR's delta type
(verdict-level vs. cosmetic-only), cross-referenced with the
coordinator's own hand-verified causal tracing (recorded in
classification-notes.json, written by hand after real source
investigation -- this script does NOT automate causal classification,
only the verdict-vs-cosmetic split, which is a mechanical JSON
comparison). Adapted verbatim from fixtures/s5-f1-r/classify-deltas.py.
"""
import json
