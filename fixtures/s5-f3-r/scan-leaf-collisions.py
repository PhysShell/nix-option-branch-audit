#!/usr/bin/env python3
"""S5-F3-R: full-corpus leaf-collision scan (adapted from
...
F2-R's own leaf-collision-scan.json -- comparison baseline is F2-R's
...
def main():
        ...
        for side in ("check-base.json", "check-head.json"):
            ...
            for t in cand["targets"]:
                ...
                findings.append({
                    ...
                    "colliding_paths_f2r_baseline": baseline_collisions,
                    "pre_existing_in_f2r_baseline": baseline_collisions == cand_collisions if baseline_collisions is not None else None,
                })
    ...
    for f in findings:
        print(f"  PR {f['pr']} ({f['side']}, target={f['target']}): {f['colliding_paths_candidate']} pre_existing={f['pre_existing_in_f2r_baseline']}")
