#!/usr/bin/env python3
"""S5-F2-R: mechanical classification of every PR changed relative to the
F1D-R baseline (not raw historical), cross-referenced with
classification-notes.json (hand-verified causal tracing, written after
real source investigation). Adapted from fixtures/s5-f1d-r/classify-deltas.py
with the comparison baseline changed to F1D-R's own replay output.
"""
import json
from pathlib import Path

D = Path(__file__).resolve().parent
F1DR_REPLAY = D.parent / "s5-f1d-r" / "replay"
...
def main():
    ...
    for r in changed:
        pr = r["pr"]
        e = manifest[pr]
        baseline_dir = F1DR_REPLAY / e["cohort"] / str(pr)
