# Migration note for programmatic/JSON consumers of `oba` (draft, v0.5.0)

This is a DRAFT compatibility note for anyone parsing `oba`'s JSON
output (`check`, `diff`, `audit`, `audit-diff`) programmatically, not
just reading its human-readable summary text. It is written to be
usable standalone (e.g. pasted into an integration's own upgrade
guide), not only in the context of this repository.
